...
# ── Bearer token (I-002) ──────────────────────────────────────────────────────
#
# Every endpoint except those listed in _PUBLIC_PATHS requires
# `Authorization: Bearer <token>` where <token> matches the contents of
# the file at AGENT_SERVICE_TOKEN_FILE.  The token is generated by
# entrypoint.sh and copied to a 0600 host tempfile by docker_manager.
#
# /health is intentionally exempt so the docker HEALTHCHECK can probe it
# without having to surface the token to the orchestrator.

_PUBLIC_PATHS: tuple[str, ...] = ("/health",)


def _load_agent_service_token() -> str | None:
    """Read the bearer token from AGENT_SERVICE_TOKEN_FILE, if set.

    Returns None when the env var is unset OR the file is missing —
    this allows local dev runs of agent_service.py outside the
    container to skip auth.  Inside the container entrypoint.sh always
    writes the file before launching the service, so production runs
    always have a token.
    """
    path = os.environ.get("AGENT_SERVICE_TOKEN_FILE")
    if not path:
        return None
    try:
        ...
        logger.warning("Failed to read AGENT_SERVICE_TOKEN_FILE=%s: %s", path, exc)
        return None


_AGENT_SERVICE_TOKEN: str | None = _load_agent_service_token()


def _require_auth(handler: "AgentHandler") -> bool:
    """Validate Authorization: Bearer <token> on the request.

    Returns True if the request is allowed to proceed.  On failure,
    sends a 401 response and returns False so the caller can short-
    circuit.  Constant-time comparison is used to avoid leaking the
    token via timing side-channels.

    Public paths (currently only /health) bypass the check so docker
    HEALTHCHECK and external orchestrator probes work without secrets.
    """
    raw_path = handler.path or ""
    bare = raw_path.split("?", 1)[0]
    if bare in _PUBLIC_PATHS:
        return True
    if _AGENT_SERVICE_TOKEN is None:
        # Token not configured — block with a clear message rather than
        # silently allowing through.  Local dev outside the container
        # should set AGENT_SERVICE_TOKEN_FILE explicitly.
        handler.send_error(401, "agent_service token not configured")
        return False
    got = handler.headers.get("Authorization", "") or ""
    if not got.startswith("Bearer "):
        handler.send_error(401, "auth required")
        return False
    presented = got[len("Bearer "):].strip()
    if not hmac.compare_digest(presented, _AGENT_SERVICE_TOKEN):
